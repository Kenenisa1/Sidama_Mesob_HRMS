...
function JobList({ mode = "explore" }) {
  ...
  // Robust parsing utility to handle both Digits (7, 8) and Roman Numerals (VIII, XIII)
  const getWindowDaysDisplay = (rankStr) => {
    if (!rankStr) return "በ 5 የሥራ ቀናት ውስጥ";
    
    // Check for high rank indicators using explicit string matching
    const isHighRank = /Level\s+(VIII|IX|X|XI|XII|XIII|XIV)/i.test(rankStr);
    if (isHighRank) {
      return "በ 10 የሥራ ቀናት ውስጥ";
    }
    
    // Fallback digit evaluation if saved as standard integers
    ...
    return (
      <div className="flex flex-wrap gap-2.5 mt-2">
        {tags.map((tag, index) => (
          <span 
            key={index} 
            className="text-sm font-bold bg-[#0a0c10] hover:bg-[#161b22] text-zinc-100 border-2 border-zinc-800 px-3.5 py-1.5 rounded-lg transition-colors tracking-wide"
          >
            {tag}
          </span>
        ))}
                  ...
                  <div className="space-y-5">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-900 pb-4">
                      <div className="font-mono text-xs text-emerald-400 uppercase tracking-wider flex flex-wrap items-center gap-2">
                        <span className={`font-black px-3 py-1 rounded-md border text-sm ${
                          isExpired 
                        ...
                        <span className="text-zinc-300 font-bold text-sm bg-zinc-900 px-2.5 py-1 rounded-md">{job.department}</span>
                      </div>
                    ...
                    <div className="space-y-2 pt-2">
                      <h3 className="text-xs uppercase font-black tracking-wider text-zinc-400 flex items-center gap-2">
                        ...
                        <span>የሚያስፈልግ የትምህርት ዝግጅት / Eligible Qualifications & Fields</span>
                      </h3>
                      <div className="bg-[#04060a] border-2 border-zinc-900 rounded-xl p-4 shadow-inner">
                        {renderFieldTags(job.eligibleFields)}
                      </div>
                    </div>

                    {job.experienceRequirements && (
                      <div className="text-sm text-zinc-200 bg-[#04060a] p-4 rounded-xl border-2 border-zinc-900 leading-relaxed font-sans shadow-inner">
                        <span className="font-black text-zinc-400 block uppercase font-mono tracking-wider mb-2 text-xs border-b border-zinc-900 pb-1">
                          Ref: የስራ ልምድ እና ሌሎች መስፈርቶች / Experience Terms:
                        </span>
                        <p className={`font-medium ${isExpired ? "text-zinc-500" : "text-zinc-300"}`}>{job.experienceRequirements}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="border-t-2 border-zinc-900 pt-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-[#020305] -mx-6 -mb-6 p-6 mt-2">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2 text-sm font-mono text-zinc-400">
                        ...
                        {/* Patched badge line to securely display lookahead timelines without causing auto-translation bugs */}
                    ...
                    <div className="flex flex-wrap items-center gap-3 justify-between md:justify-end">
                      ...
                      {isExpired ? (
                        ...
                        <button 
                          ...
                          className="w-full md:w-auto bg-emerald-600 hover:bg-emerald-500 text-white font-black text-sm uppercase tracking-wider py-3.5 px-6 rounded-xl transition-all text-center shadow-[0_4px_20px_rgba(16,185,129,0.15)] hover:scale-[1.02]"
                        >
        ...
        {mode === "home" && !loading && jobs.length > 0 && (
          <div className="flex justify-center pt-6">
            <button
              ...
              className="group bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs uppercase tracking-widest py-4 px-10 rounded-xl shadow-[0_4px_25px_rgba(16,185,129,0.2)] flex items-center gap-2 transition-all hover:scale-[1.01]"
            >
