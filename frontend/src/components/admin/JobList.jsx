...
function JobList({ mode = "explore" }) {
  ...
  const renderFieldTags = (fieldsString) => {
    if (!fieldsString)
      return (
        <span className="text-xs font-mono text-[var(--color-text-secondary)] italic">
          No specific field constraints specified.
        </span>
      );
    ...
    return (
      <div className="flex flex-wrap gap-2">
        {tags.map((tag, index) => (
          <span
            key={index}
            className="text-[11px] font-mono font-bold bg-[var(--color-surface)] text-[var(--color-text-secondary)] border border-[var(--nav-border)] px-2.5 py-1 rounded-md tracking-wide"
          >
            {tag}
          </span>
        ))}
  ...
  return (
    <div
      className={`text-[var(--color-text-primary)] font-sans selection:bg-emerald-500/20 selection:text-emerald-400 ${mode === "explore" ? "min-h-screen p-3 sm:p-6 md:p-12 bg-[var(--bg)]" : "py-10"}`}
      id="jobs"
    >
      <div className="max-w-5xl mx-auto space-y-10">
        {mode === "explore" ? (
          <>
            <header className="border-b-2 border-[var(--nav-border)] pb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              {/* <div className="bg-zinc-950/80 border border-zinc-800 p-3 rounded-xl max-w-sm text-[11px] font-mono text-zinc-400 leading-relaxed">
                 <strong className="text-[var(--color-text-primary)]">የምዝገባ ቦታ፦</strong> ወልደአማኑኤል ስታዲየም አጠገብ፣ በአሮጌው ገበያ ወደ ውስጥ በሚወስደው መንገድ ላይ፣ በሲዳማ መሶብ የሰው ኃይል ልማት ዳይሬክቶሬት 4ኛ ፎቅ።
              </div> */}
            </header>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-[var(--color-surface)] p-5 rounded-2xl border-2 border-[var(--nav-border)] shadow-xl">
              <div className="relative md:col-span-2">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-secondary)] w-5 h-5" />
                <input
                  type="text"
                  placeholder={t("jobs.searchPlaceholder")}
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full bg-[var(--color-surface)] border-2 border-[var(--nav-border)] rounded-xl py-3.5 pl-12 pr-4 text-[var(--color-text-primary)] text-base font-medium focus:outline-none focus:border-emerald-500 transition-all placeholder:text-[var(--color-text-secondary)] shadow-inner"
                />
              </div>

              <div className="relative">
                <Filter className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-secondary)] w-5 h-5" />
                <select
                  value={selectedDepartment}
                  onChange={(e) => setSelectedDepartment(e.target.value)}
                  className="w-full bg-[var(--color-surface)] border-2 border-[var(--nav-border)] rounded-xl py-3.5 pl-12 pr-10 text-[var(--color-text-secondary)] text-base font-bold focus:outline-none focus:border-emerald-500 transition-all cursor-pointer appearance-none"
                >
                  ...
                  <option value="Planning & Budgeting">
                    Planning & Budgeting (ዕቅድና በጀት)
                  </option>
                  <option value="Public Relations">
                    Public Relations (ህዝብ ግንኙነት)
                  </option>
                  <option value="Technical Services">
                    Technical Services (ቴክኒካል አገልግሎት)
                  </option>
                </select>
          ...
          <div className="flex items-center justify-between border-b border-[var(--nav-border)] pb-4">
            <div>
              <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-[var(--color-text-primary)] uppercase flex items-center gap-2">
                {t("jobs.newest")}
              </h2>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-28 space-y-4">
            ...
            <p className="text-xs font-mono text-[var(--color-text-secondary)] uppercase tracking-widest font-bold">
              {t("jobs.loading")}
            </p>
          </div>
        ) : filteredJobs.length === 0 ? (
          <div className="text-center py-24 bg-[var(--color-surface)] border-2 border-dashed border-[var(--nav-border)] rounded-2xl">
            <p className="text-base text-[var(--color-text-secondary)] font-mono font-bold uppercase tracking-wider">
              {t("jobs.noMatch")}
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-5">
                    {/* Header Line Info */}
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--nav-border)] pb-4">
                      <div className="font-mono text-xs text-emerald-400 uppercase tracking-wider flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-[var(--color-text-secondary)] font-bold text-xs sm:text-sm bg-[var(--color-surface)] border border-[var(--nav-border)] px-2.5 py-1 rounded-md">
                          {job.department}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {job.employmentType && (
                          <span className="text-[10px] font-mono bg-[var(--color-surface)] text-[var(--color-text-secondary)] border border-[var(--nav-border)] px-2 py-1 rounded-md uppercase font-bold">
                            {job.employmentType}
                          </span>
                        )}
                        {isExpired ? (
                    ...
                    <div className="space-y-1.5">
                      ...
                      {job.titleAmharic && (
                        <p
                          className={`text-lg sm:text-xl font-bold tracking-normal ${isExpired ? "text-[var(--color-text-secondary)]" : "text-[var(--color-text-secondary)]"}`}
                        >
                          {job.titleAmharic}
                        </p>
                      )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 bg-[var(--color-surface)] p-4 rounded-xl border border-[var(--nav-border)] shadow-inner">
                      <div className="space-y-1">
                        <span className="text-[11px] font-mono text-[var(--color-text-secondary)] uppercase block font-black">
                          {t("jobs.rank")}
                        </span>
                        <div className="flex items-center gap-2 text-[var(--color-text-primary)]"></div>
                      </div>

                      <div className="space-y-1">
                        <span className="text-[11px] font-mono text-zinc-500 uppercase block font-black">
                          {t("jobs.salary")}
                        </span>
                        <div
                          className={`flex items-center gap-2 ${isExpired ? "text-[var(--color-text-secondary)]" : "text-emerald-400"}`}
                        >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 pt-2">
                      {job.educationLevel && (
                        <div className="md:col-span-2 space-y-2">
                          ...
                          <div className="bg-[var(--color-surface)] border border-[var(--nav-border)] rounded-xl p-3 h-[72px] flex items-center shadow-inner">
                      ...
                      >
                        <h3 className="text-xs uppercase font-black tracking-wider text-[var(--color-text-secondary)] flex items-center gap-2">
                          <GraduationCap
                            size={16}
                            className={
                              isExpired ? "text-zinc-600" : "text-emerald-500"
                            }
                          />
                          <span>{t("jobs.fields")}</span>
                        </h3>
                        <div className="bg-[var(--color-surface)] border border-[var(--nav-border)] rounded-xl p-3 min-h-[72px] flex items-center shadow-inner">
                    ...
                    {job.experienceRequirements && (
                      <div className="text-sm text-[var(--color-text-primary)] bg-[var(--color-surface)] p-4 rounded-xl border border-[var(--nav-border)] leading-relaxed font-sans shadow-inner">
                        <span className="font-black text-[var(--color-text-secondary)] block uppercase font-mono tracking-wider mb-2 text-xs border-b border-[var(--nav-border)] pb-1 flex items-center gap-1.5">
                          <Briefcase
                            size={13}
                        ...
                        <p
                          className={`font-medium whitespace-pre-wrap text-xs sm:text-sm ${isExpired ? "text-[var(--color-text-secondary)]" : "text-[var(--color-text-secondary)]"}`}
                        >
                          {job.experienceRequirements}
                        </p>
                      </div>
                    ...
                    {job.additionalNotes && (
                      <div className="text-sm text-[var(--color-text-primary)] bg-[var(--color-surface)]/80 p-4 rounded-xl border border-dashed border-[var(--nav-border)] leading-relaxed font-sans shadow-inner">
                        <span className="font-black text-[var(--color-text-secondary)] block uppercase font-mono tracking-wider mb-1 text-xs flex items-center gap-1.5">
                          <FileText size={13} /> {t("jobs.notes")}
                        </span>
                        <p className="font-medium text-[var(--color-text-secondary)] text-xs whitespace-pre-wrap">
                          {job.additionalNotes}
                        </p>
                      </div>
                    )}
                  ...
                  <div className="border-t-2 border-[var(--nav-border)] pt-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-[var(--color-surface)] -mx-6 -mb-6 p-6 mt-2">
                    <div className="space-y-1">
                      <span className="text-[10px] font-mono text-[var(--color-text-secondary)] uppercase block font-black">
                        {t("jobs.deadline")}
                      </span>
                      <div className="flex flex-wrap items-center gap-2 text-sm font-mono text-[var(--color-text-secondary)]">
                        <Calendar
                          size={15}
                          className="shrink-0 text-rose-500"
                        />
                        <span
                          className={`text-base font-black ${isExpired ? "text-rose-600 line-through" : "text-rose-400"}`}
                        >
                    ...
                    <div className="flex flex-wrap items-center gap-3 justify-between md:justify-end">
                      {job.requiresCOC && (
                        <span
                          className={`text-xs font-mono border-2 px-3 py-1.5 rounded-xl uppercase tracking-wider font-black ${
                            isExpired
                              ? "bg-[var(--color-surface)]/50 text-[var(--color-text-secondary)] border-[var(--nav-border)]"
